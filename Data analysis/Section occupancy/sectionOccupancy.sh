...
grep "^${PREFIX}${EXPERIMENT_ID} " "$CONFIG_FILE" | while read -r EXP_ID DAY_LABEL START_FILE END_FILE; do
    ...
    python3 "$SCRIPT" "$START_FILE" "$END_FILE" "$MISSING_ANIMALS" > "${OUTDIR}/${DAY_LABEL}.txt"
    python3 "$SCRIPT" "$START_FILE" "$END_FILE" "$MISSING_ANIMALS" "--naive" > "${OUTDIR}/${DAY_LABEL}_Naive.txt"
